  ...
  <div class="report-result">
    <div class="result-card" ref="pdfContent">
      <div class="header-row">
        <div class="saved-at" v-if="savedAt">
          Saved At: {{ formatDate(savedAt) }}
        </div>
      ...
      <section
        v-if="comparisonTable && comparisonTable.length"
        ...
        <div class="table-wrapper">
          <table>
            <thead>
              <tr>
                <th>Section</th>
                <th v-for="company in companyNames" :key="company">
                  {{ company }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="header in tableHeaders" :key="header">
                <td>{{ header }}</td>
                <td v-for="company in companyNames" :key="company">
      ...
      <section class="main-report" v-if="mainReport">
        ...
        <div
          v-for="(content, title) in mainReport"
          :key="title"
          class="info-box"
        >
          <h4>{{ title }}</h4>
      ...
      >
        ...
        >
          ...
          <div
            v-for="(content, title) in comp.sections"
            :key="title"
    ...
    <div class="reanalyze-wrapper no-pdf">
      <button class="reanalyze-btn" @click="$emit('reanalyze')">
        Re-Analyze
      </button>
    </div>
  </div>
  ...
  computed: {
    ...
    tableHeaders() {
      if (this.comparisonTable.length > 0) {
        return Object.keys(this.comparisonTable[0]).filter(
          (k) => k !== "company"
        );
      }
      return [];
